...
function renderOptions(poll, management = false) {
  ...
  return poll.options.map(option => {
    ...
    const draft = otherDrafts.get(String(poll.id)) || '';
    const otherInput = option.other && !management && poll.open
      ? `<input class="other-input" data-other-input="${poll.id}" maxlength="200" placeholder="Write your own opinion or option…" aria-label="Your Other option" value="${escapeHtml(draft)}">`
      : '';
...
async function vote(pollId, optionId) {
  ...
  const customText = optionId === -1 ? (otherDrafts.get(String(pollId)) || '').trim() : null;
  if (optionId === -1 && !customText) {
    error.textContent = 'Write your own opinion for Other.';
    document.querySelector(`[data-other-input="${pollId}"]`)?.focus();
    return;
  }
  try {
    ...
    if (optionId === -1) {
      otherDrafts.delete(String(pollId));
    }
...
async function closePoll(id) {
  if (!confirm('Close this poll? Voting will stop immediately.')) {
    return;
  }
  try {
...
async function deletePoll(id) {
  if (mode !== 'admin' || !confirm('Delete this poll permanently?')) {
    return;
  }
  try {
...
$('optionFields').addEventListener('click', event => {
  ...
  if (button) {
    removeOption(Number(button.dataset.removeOption));
  }
});
...
$('polls').addEventListener('click', event => {
  ...
  if (button.dataset.action === 'vote') {
    vote(Number(button.dataset.poll), Number(button.dataset.option));
  }
...
$('polls').addEventListener('input', event => {
  ...
  if (event.target.classList.contains('other-input')) {
    otherDrafts.set(String(event.target.dataset.otherInput), event.target.value);
  }
});
